...
const ImageUpload = ({
  ...
  const handleImagePicker = async (option) => {
    ...
    setTimeout(async () => {
      try {
        ...
        if (!result.canceled && result.assets && result.assets[0]) {
          ...
          // ✅ normalize file object here
          const normalized = {
            uri: asset.uri,
            type: asset.mimeType || (asset.type?.startsWith("image") ? "image/jpeg" : asset.type),
            name: asset.fileName || `upload_${Date.now()}.jpeg`,
          };

          console.log("Normalized image object:", normalized);
    ...
    }, 600); // add delay, to make sure that the modal fully disappears
  };
    ...
    <View>
      ...
      >
        {value ? (
          ...
          >
            <Ionicons name="person" size={size * 0.4} color="#C2000E" />
          </View>
        )}
