...
export default function DeliveryEdit() {
  ...
  useEffect(() => {
    const fetchDeliverySetting = async () => {
        ...
        const response = await fetch(
            `${VITE_API_BASE_URL}delivery-settings/show/${id}`,
            {
            method: "GET",
            headers: {
        ...
        if (response.ok && result.data) {
            setFormData({
            deliveryRangeStart: result.data.start_km || '',
            deliveryRangeEnd: result.data.end_km || '',
            pricePerKm: result.data.price_per_km || '',
            minimumPurchase: result.data.min_purchase_discount || '',
            discountAmount: result.data.discount_amount || '',
            });
        } else {
            toast.error(result.message || "Failed to fetch delivery setting.");
        }
        } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch(
        `${VITE_API_BASE_URL}delivery-settings/update/${id}`,
        {
          method: "POST",
          headers: {
