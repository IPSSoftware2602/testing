...
export default function WalletBalanceHistory() {
  useAuthGuard();
  ...
  const getCustomerWallets = async () => {
    if (customerData?.id) {
      try {
        const response = await axios.get(
          `${apiUrl}customers/wallet/${customerData?.id}`,
          {
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${authToken}`,
  ...
  const renderEmptyWalletHistory = () => (
    <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', paddingVertical: 40 }}>
      ...
      <Text style={{
        ...
        You don&apos;t have wallet transactions yet.
      </Text>
    </View>
  );
  ...
  return (
    <ResponsiveBackground>
      <SafeAreaView style={{ flex: 1, backgroundColor: '#FFF' }}>
        <TopNavigation title="WALLET HISTORY" isBackButton={true} navigatePage={() => router.push('(tabs)/profile')} />
            ...
            <View style={styles.profileRow}>
              ...
              <View style={{ width: '40%', flexDirection: 'column', alignItems: 'flex-end', justifyContent: 'flex-end', marginTop: 10 }}>
                <TouchableOpacity style={{ flexDirection: 'column', alignItems: 'flex-end' }}>
                  <Text style={styles.pointsValue}>RM</Text>
                  {customerWallet ? (
                    <Text style={styles.pointsLabel}>  {customerWallet}</Text>
                  ) : (
                    <Text style={styles.pointsLabel}>  888</Text>
                  )}
                </TouchableOpacity>
                <Text style={styles.streakText}>Transaction details breakdown</Text>
              </View>
            </View>
          </LinearGradient>
        </View>
        ...
        <View style={styles.checkinCard}>
          <Text style={styles.checkinTitle}>Wallet History Details</Text>
          ...
          <FlatList
            ...
            ListEmptyComponent={renderEmptyWalletHistory}
...
const styles = StyleSheet.create({
  ...
  flatList: {
    maxHeight: height * 0.6, // Optional: limit height for scroll area
  },
}); 